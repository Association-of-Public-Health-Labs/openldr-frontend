...
export default function SamplesTestedByPregnancy() {
  ...
  const handleChartClick = (value) => { 
    ...
    if (chartType === "province") {
      ...
      setSQLQuery(`AnalysisDatetime >= '${dates[0]}' AND AnalysisDatetime <= '${dates[1]}' AND RequestingFacilityName='${value}' AND ViralLoadResultCategory IS NOT NULL AND Pregnant IN ('Yes','YES','yes','Sim','SIM','sim')`)
      setLocation(value)
      setVisible(true)
    }
  };
