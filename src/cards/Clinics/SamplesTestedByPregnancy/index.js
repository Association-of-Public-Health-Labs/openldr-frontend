...
export default function SamplesTestedByPregnancy() {
  ...
  async function exportRawData (healthFacility) {
    ...
    const query = `AnalysisDatetime >= '${dates[0]}' AND AnalysisDatetime <= '${dates[1]}' AND RequestingFacilityName='${healthFacility}' AND ViralLoadResultCategory IS NOT NULL AND Pregnant IN ('Yes','YES','yes','Sim','SIM','sim')`
