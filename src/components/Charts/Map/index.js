...
export default function Map({ data, labels }) {
    ...
    async function loadData() {
      ...
      data.map((province) => {
        if (province.province === "Niassa") {
          setNiassa(province);
          tippy("#niassa", {
            content: `<strong>${province.province}</strong><br/><span>VL Suppresion rate: ${province.suppressed}</span><br/><span>Routine: ${province.routine}</span><br/><span>Treatment Failure: ${province.treatment_failure}</span>`,
            allowHTML: true,
            distance: 0,
            delay: 300,
            theme: "tooltip",
